(function() {
  ...
  _.last = function(array, n) {
    //TODO: Find a better way to do this with ternary's
      
      /*If n is undefined, the last element is returned.
       *Else if n is less than the length of the array, the last n elements are returned.
  ...
  _.each = function(collection, iterator) {
    ...
    if (Array.isArray(collection)) {
      //if it's an array, iterate over collection using a for loop invoking the iterator on each element.
      for(var i = 0; i < collection.length; i++) {
        //Invoke the iterator with the element index and collection.
        iterator(collection[i], i, collection);
      }
    }
      //Iterate using for in loop if collection is an object,
    else {
  ...
  _.filter = function(collection, test) {
    //Empty array will populate with all items that pass the truth test.
      var result = [];

    //Iterate through the collection using each.
    _.each(collection, function(item, index, collection) {
      //Test if each item passes a truth test
      if (test(item, index, collection)) {
        //If so, the collection item is pushed to the results array.
        result.push(item);
      }
    });
  ...
  _.reject = function(collection, test) {
    ...
    // copying code in and modifying it
     return _.filter(collection, function (item, index, collection) {
      //Return items if they fail the test.
  ...
  _.extend = function(obj) {
    ...
    for (var i = 1; i<argArr.length;i++){
      ...
      _.each(argArr[i], function(value, key) {
          ...
          //Adding each key value pair from the argument object to the orginal object to be extended.
          obj[key] = value;
        });     
    }
    return obj;
  ...
  _.delay = function(func, wait) {
    ...
    //Coverting the arguments to an array and slicing it to only contain from the 2nd element on to pass into the func.
    var args = Array.prototype.slice.call(arguments,2);

    //Calling the func with the arguments passed at a delay using setTimeout.
    setTimeout(function() {
